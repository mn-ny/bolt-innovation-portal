...
export default function SponsorsSection() {
  ...
  return <section id="sponsors-section" className="py-16 relative z-10">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className={`grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8 items-center justify-items-center ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"} transition-all duration-700 delay-300 ease-out`}>
          {sponsors.map((sponsor, index) => <div key={index} className="flex items-center justify-center">
              <img src={sponsor.logo} alt={`${sponsor.name} logo`} className="sponsor-image object-contain" />
            </div>)}
        </div>
      </div>
    </section>;
